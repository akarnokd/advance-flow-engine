...
public class Timer extends AdvanceBlock {
	...
	public void init(AdvanceBlockDescription desc,
		...
		int inits = 0;
		if (constantParams.containsKey("delay")) {
			interval = XData.getInt(constantParams.get("delay").value);
			inits++;
		}
		if (constantParams.containsKey("in")) {
			last.set(constantParams.get("in").value);
			inits++;
		}
		if (inits == 2) {
			startTimer();
